...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        # The legacy project used a cwd-relative checkpoint. Stage_4_train.py
        # supplies an explicit path and still permits a no-weight smoke test.
        load_path = getattr(cfg, 'backbone_path', None) or './pvt_v2_b4.pth'
        if load_path and os.path.isfile(load_path):
            ...
            print(f'warning: PVT-v2 checkpoint not found at {load_path}; training from random initialization')
        """by_chf:512?"""
    ...
    def forward(self, x, shape=None, epoch=None):
        ...
        bk_stage5,bk_stage4,bk_stage3,bk_stage2=self.bkbone(x)#bk_stage5=[8,512,6,6]
        ...
        # F4 = F4
        # F3 = self.upsample_add(F4,F3)
        # F2 = self.upsample_add(F3,F2)
        # F1 = self.upsample_add(F2, F1)
