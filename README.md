...
│   └── bekd_wscod_reference/   # BeKD_WSCOD_2 student-network source (no weights/data)
...
Stage 3 reads `SAM_Masks/multi_mask/` directly and writes one selected, merged mask per image to `Quality_Masks/High/`, `Quality_Masks/Medium/`, or `Quality_Masks/Low/`.
...
- Otherwise: inspect the selected candidate's edge-truncation count and connected-component count. At most one touched edge and at most 49 components produce **Medium**; otherwise the mask is downgraded to **Low**.

The selected candidate is the one with the highest SAM predicted-IoU in its filename. For images containing multiple boxes, their selected masks are merged. Any Low-quality box makes the final merged image Low; otherwise any Medium-quality box makes it Medium. `Quality_Masks/manifest.jsonl` records the metrics, selected candidate, and tier for every box.
...
Stage 4 uses the Stage 3 tier in `Quality_Masks/manifest.jsonl`, the selected final mask, all three Stage 2 SAM candidates, and the Stage 1 bounding boxes. It follows the useful processing conventions retained in `Refer/` and `BeKD_WSCOD_2`:

- The three candidate masks are merged per candidate index for multi-box images.
- Their mean vote is converted to a binary-entropy confidence map, then optionally Gaussian-smoothed, following `Refer/fuse_mask.py` and `Refer/guass_weight.py`.
- `Pseudo_Label/` uses the legacy coding of `1` for foreground and `2` for background.
- `Box_Background/` labels only bbox-exterior pixels as background (`2`); bbox interiors are `0`/ignore. This deliberately avoids the inverted box-fill experiment in `Refer/box_tos.py`.
...
├── train.txt
└── train/
    ├── Imgs/                    # Hard links to input images (copies if linking is unavailable)
    ├── Pseudo_Label/            # Selected Stage 3 masks, encoded as 1/2
    ├── Box_Background/          # Bbox-exterior reliable background, encoded as 2
    ├── Pixel_Weight/            # Refer-style candidate entropy confidence
    └── Candidate_0|1|2/         # Merged SAM candidate masks
```

`Stage_4_train.py` exposes the three retained BeKD-style branches: High uses weighted BCE plus L1/MSE, Medium uses `0.5 ×` weighted BCE, and Low uses `0.1 ×` augmentation-based self-distillation. Every branch additionally uses the reliable bbox-exterior background map. The bundled student source comes from `BeKD_WSCOD_2`; its hard-coded GPU selection and temporary fixed-mask paths have been removed from the new entry point.
...
The checkpoint is recommended but optional for a smoke test. Stage 4 requires `timm` and `tensorboardX`, both listed in `requirements.txt`. Training checkpoints are written to `Stage4_Checkpoints/`.
